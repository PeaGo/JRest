...
class Validator
{
    ...
    public function __construct()
    {
        Factory::setDefaultInstance((new Factory())
            ->withRuleNamespace('JRest\\Validation\\Rules')
            ->withExceptionNamespace('JRest\\Validation\\Exceptions'));
    }
    public function validate(ServerRequestInterface $request, array $rules)
    {
        ...
        foreach ($rules as $field => $rule) {
            try {
                $rule->setName($field)->assert($request->getQueryParams($field));
